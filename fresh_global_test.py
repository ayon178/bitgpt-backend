#!/usr/bin/env python3
"""
Fresh Global Program Test - 3 Key Requirements
1. First user automatic phase 2 te jacche kina tar under a 4 jon hole
2. Second user perfectly phase 2 te first user er under a bosche kina second user er under a 4 jon join hole  
3. First user phase 2 theke phase 1 a slot 2 te upgrade hocche kina first user er under a 8 jon user asle phase 1 theke
"""
...
def create_fresh_user(name_suffix):
    """Create a fresh test user"""
    try:
        timestamp = int(datetime.utcnow().timestamp())
        email = f"fresh_test_{name_suffix}_{timestamp}@example.com"
        
        # Create new test user
        user = User(
            uid=f"FRESH_{name_suffix}_{timestamp}",
...
def main():
    print("🚀 Fresh Global Program Test - 3 Key Requirements")
    print("=" * 60)
    print("1. First user automatic phase 2 te jacche kina tar under a 4 jon hole")
    print("2. Second user perfectly phase 2 te first user er under a bosche kina second user er under a 4 jon join hole")
    print("3. First user phase 2 theke phase 1 a slot 2 te upgrade hocche kina first user er under a 8 jon user asle phase 1 theke")
    print("=" * 60)
    
    # Step 1: Create users
    print("\n📝 Step 1: Creating fresh test users...")
    users = []
    
    # Create enough users to test all scenarios (at least 12 users)
    for i in range(15):
        user_id = create_fresh_user(f"User{i+1}")
        if user_id:
