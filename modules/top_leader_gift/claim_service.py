...
class TopLeadersGiftClaimService:
    ...
    def get_claim_history(self, user_id: str) -> Dict[str, Any]:
        """Get simplified claim history - only claims array with level, currency, amount, time, date"""
        try:
            # Get user
            tl_user = TopLeadersGiftUser.objects(user_id=ObjectId(user_id)).first()
            if not tl_user:
            ...
            # Get all payment records for this user (only successful ones)
            payments = TopLeadersGiftPayment.objects(
                user_id=ObjectId(user_id),
                payment_status='paid'
            ).order_by('-created_at')
            
            # Build simplified claims array
            claims = []
            for payment in payments:
                claim_obj = {
                    ...
                    "amount": {
                        "usdt": payment.claimed_amount_usdt,
                        "bnb": payment.claimed_amount_bnb
                    },
                    "time": payment.created_at.strftime("%I:%M %p") if payment.created_at else None,
