#!/usr/bin/env python3
"""
Global Program Testing with Existing Users using Wallet Login
"""
...
# Existing users with their wallet addresses from previous temp-create responses
EXISTING_USERS = [
    {
...
def login_with_wallet(wallet_address: str) -> str:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Token: {str(token)[:50]}..." if token else "   No token received")
            return token
        else:
...
def test_global_program():
    """Main test function"""
    print("🚀 Starting Global Program Testing with Existing Users (Wallet Login)...")
    print("=" * 60)
    
    # Step 1: Login users with wallet addresses
    print("\n🔐 Step 1: Logging in users with wallet addresses...")
    user_tokens = {}
    ...
    for user_info in joined_users[:3]:  # Test first 3 users
        user_id = user_info["user_id"]
        name = user_info["name"]
        token = user_tokens.get(user_id)
    ...
    for user_info in joined_users[:2]:  # Test first 2 users
        user_id = user_info["user_id"]
        name = user_info["name"]
        token = user_tokens.get(user_id)
